...
- **Service & Guard Layer**: Modular services (`services/`), interaction dispatchers (`interactions/`), and voice guards (`utils/voiceGuard.js`).
...
```
discord-bot-v2/
├── index.js                    # Entry point: Client init, player manager loading, dynamic loaders, health server
...
│   ├── messageService.js       # Auto-deleting temporary reply and channel message helpers
│   ├── youtubeService.js       # YouTube API search & autocomplete suggestions
│   ├── geminiService.js        # Gemini AI DJ playlist generation service
│   └── playerManager.js        # Moonlink Audio Manager init, event listeners & idle disconnect timer
├── interactions/               # Sub-interaction handlers
│   ├── buttons/
│   │   ├── playerButtons.js    # Pause, resume, skip, stop, queue, loop button handlers
│   │   └── queuePagination.js # Queue page navigation & refresh button handlers
│   └── autocomplete/
│       └── playAutocomplete.js# Debounced autocomplete handler
├── commands/
│   ├── music/
│   │   ├── play.js             # Play track/playlist with search or URL
│   │   ├── dj.js               # Gemini-powered AI DJ playlist generator
│   │   ├── skip.js             # Skip current track(s)
│   │   ├── stop.js             # Stop playback, clear queue, disconnect
│   │   ├── queue.js            # View active queue embed
│   │   └── nowplaying.js       # View current track & visual progress
│   └── utility/
│       └── ping.js             # Bot latency & websocket ping command
├── events/
│   ├── interactionCreate.js    # Modular event router for ChatInput, Buttons, and Autocomplete
...
- Do not duplicate voice channel checks in commands or button handlers. Use `validateVoicePermissions(interaction, client, options)`:
  ...
  const voiceState = await validateVoicePermissions(interaction, client, { requirePlayer: true, requirePlaying: true });
