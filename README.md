...
A feature-rich, modern Discord music bot built with **Discord.js v14**, **Moonlink.js**, **NodeLink / Lavalink**, and powered by **Google Gemini AI** for AI-generated DJ playlists.
...
| `/skip` | None | Skip the current playing track. |
| `/stop` | None | Stop playback, clear the queue, and leave the voice channel. |
| `/queue` | None | View current playing track and queued songs. |
...
│   ├── music/           # Music playback & AI DJ commands (play, dj, skip, stop, queue)
│   └── utility/         # General utility commands (ping)
├── events/              # Discord event handlers (interactionCreate, ready, voiceStateUpdate, messageCreate)
├── utils/               # Design tokens, custom embeds, and component builders
│   ├── theme.js         # Color palette & formatting helpers (progress bar, duration)
...
├── index.js             # Main entry point (Client setup, Moonlink manager, health server)
